...
- `generator` is provenance: the tool you are running in (`engine`) and, if your harness told you what model you are (system prompt, the model you were launched with), that model ID as given. Leave `model` out rather than guess; readers see it as "generated by". Optional.
